...
def image_creator(selected,np_instance_map,np_img):
    new_inst=np.zeros_like(np_instance_map)
...
def processor(filename,img,instance_map,instance_dir,image_dir):
    ...
    for i in range(num_samples):
        ...
        img_new_inst=Image.fromarray(new_inst, 'P')
        img_new_img=Image.fromarray(new_img,'RGB')
        img_new_inst.save( os.path.join( instance_dir,filename  )  )
        img_new_img.save( os.path.join( image_dir, filename   )   )


for (dirpath,dirnames,filenames) in os.walk(wd_fine):
    for filename in filenames:
        if 'gtFine_instanceIds' in filename:
            instance_map=Image.open(filename)
            ...
            processor(dirpath,filename,img,instance_map)

            instance_dir=dirpath.replace('gtFine','instances')
            image_dir = dirpath.replace('gtFine','images')
